...
public class Cracker {
	...
	public static void c_vigenère(String codedMessage) {
		...
		for (int i = 0; i < Dictionary.dictionary.size(); i++) {
			x = scanForWords(Decrypter.d_vigenère(codedMessage, Dictionary.dictionary.get(i)));
			if (x > bestNum) {
				bestMatch = Decrypter.d_vigenère(codedMessage, Dictionary.dictionary.get(i));
			}
		}
		System.out.println("Best decryption from dictionary:  " + bestMatch + "\nTesting all non-existent 3 letter words.");
		while (true) {
			x = scanForWords(Decrypter.d_vigenère(codedMessage, currWord));
			if (x > bestNum) {
				bestMatch = Decrypter.d_vigenère(codedMessage, currWord);
			}
